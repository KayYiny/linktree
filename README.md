...
- 🎨 **品牌图标库**：链接图标可在内嵌的 Simple Icons 图标库弹窗中按品牌名一键选取（无需手填 URL）
...
├── assets/
│   ├── scripts/          # keygen / easter-egg / hint-popup / gallery / pet / i18n / qrcode-popup
...
- **链接图标**：在内嵌的 Simple Icons 图标库弹窗中搜索品牌、一键填入图标 URL（弹窗带加载指示，网络慢时先显示加载动画）。
